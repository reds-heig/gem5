...
class PciBridge(ClockedObject):
    ...
    is_switch = Param.Int(
        0,
        "Whether this is a switch and virtual p2p"
        "bridge needed for upstream port",
    )
...
class Root_Complex(PciBridge):
    ...
    # Enable bus mastering, intx gen,memory address space decoder,
    # io address space decoder
    Command = 0x0407
    # Indicate that capability ptr has been implemented
    Status = 0x0010
    ...
    # Max payload 256B
    # L0 acceptable latency: 64-128ns
    # L1 acceptable latency 1-2 us
    PXCAPDevCapabilities = 0x00000241
    PXCAPDevCtrl = (
        0x1020  # Max payload size : 256B. Max read request size : 256B
    )
    # Max link width : 6'b1
    # Max link speed : 2.5 Gbps
    # ASPM:00
    # L0s exit latency: 64-128 ns
    # L1 exit latency 1-2 us
    # Port number : 1.
    PXCAPLinkCap = 0x01009011
    PXCAPDevStatus = 0x000
    # Root ports:128 B Read Completion Boundary(RCB) support
    # ASPM disabled=> Bits 1 and 0 are just 0.
    PXCAPLinkCtrl = 0x0008
    PXCAPLinkStatus = (
        0x0011  # Negotiated link width : 1 , Negotiated link speed : 2.5 Gbps
    )

    # Need to implement (slot??) registers and root registers for a root port.
    # Assume that drivers are not needed for Root Port detection
    # and Configuration (Assume that kernel bus drivers do this).
    # Also need to assign bridge params to the PciBridge.
    # So, master-port, slave-port and latencies need to be assigned.
    # Assume bridges can raise interrupts (not required ??).
